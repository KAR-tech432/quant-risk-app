...
st.caption("Combining Fundamentals, Microstructure, Smart Money Flow & Quantitative Risk Controls")
...
if df is None or df.empty:
    ...
    # -------------------------------------------------------------
    # ALGORITHMIC DECISION ENGINE MATRIX
    # -------------------------------------------------------------
    bullish_score = 0
    bearish_score = 0

    if curr_price > sma_200_val: bullish_score += 2
    else: bearish_score += 2

    if obv_slope_val > 0: bullish_score += 2
    else: bearish_score += 2

    if rsi_val > 50 and rsi_val < 70: bullish_score += 1
    elif rsi_val >= 70: bearish_score += 1 # Overbought warning
    elif rsi_val <= 30: bullish_score += 1 # Oversold opportunity

    if prob_up >= 55: bullish_score += 2
    elif prob_up <= 45: bearish_score += 2

    pe_ratio = info.get('trailingPE', 0)
    if pe_ratio and pe_ratio < 25: bullish_score += 1

    # Decision Output Mapping
    if bullish_score >= 6 and z_score_val < 1.8:
    ...
    elif bearish_score >= 6 or z_score_val > 2.2:
    ...
    # -------------------------------------------------------------
    # 2. FUNDAMENTAL HEALTH & VALUATION METRICS
    # -------------------------------------------------------------
    st.subheader("🏛️ Pillar 1: Fundamental Ratios & Valuation")
    ...
    # -------------------------------------------------------------
    # 3. MICROSTRUCTURE & INSTITUTIONAL MONEY FLOW
    # -------------------------------------------------------------
    st.subheader("⚡ Pillar 2 & 3: Microstructure & Institutional Flow")
    ...
    q4.metric("XGBoost Directional Edge", f"{prob_up:.1f}% Bullish", f"Model Acc: {accuracy:.1f}%")
    ...
    # -------------------------------------------------------------
    st.subheader("🛡️ Pillar 4: Risk Control & Sizing Matrix")
    ...
    # -------------------------------------------------------------
    # 5. CHARTS & FINANCIAL STATEMENTS TABS
    # -------------------------------------------------------------
    tab1, tab2, tab3 = st.tabs(["📊 Price Action & Control Points", "🏦 Shareholding Pattern", "📜 Quarterly Financials"])
